...
def process_replies(db, user_id: int):
    ...
    for email in emails:
        ...
        if any(word in body.lower() for word in AMBIGUOUS_WORDS):
            print("⚠️ Ambiguous phrase detected → calling LLM")
            llm_result = llm_extract_intent_and_time(body)
        ...
        if llm_result and llm_result.get("date") and llm_result.get("time"):
            final_intent = {
        ...
        # DETERMINE TIME
        # -----------------------------
        if final_intent["intent"] == "CLIENT_PROVIDED_TIME":

            if final_intent.get("source") == "LLM":
                print("🤖 USING LLM DATE")

                start_time = datetime.strptime(
                    f"{final_intent['date']} {final_intent['time']}",
                    "%Y-%m-%d %H:%M"
                )
                start_time = IST_TZ.localize(start_time)

                client_time = final_intent["time"]
                client_timezone = final_intent.get("timezone")
        ...
        elif final_intent["intent"] == "ASKED_TO_SCHEDULE":
            start_time = datetime.now(IST_TZ) + timedelta(minutes=30)
            client_time = None
            client_timezone = None
        ...
        print("✅ Meeting scheduled using LLM")
